nv.models.radar = function() {
    ...
    chart.radiusFix = function(_) {
        if (!arguments.length) return radiusFix;
        radiusFix = _;
        return chart;
    };
    ...
    function chart(selection) {
        selection.each(function(data) {
           ...
           /* var that = this;
            ...
            radius = (availableWidth-radiusFix >= availableHeight) ? (availableHeight)/2 : (availableWidth-radiusFix)/2;
            ...
            if (data && data.length>0){
                radars.color(data.map(function(d,i) {
